        ...
        <div className="space-y-6">
            ...
            <div className="bg-gray-900 border border-amber-500/30 rounded-lg shadow-md">
                <div className="flex flex-row items-center justify-between p-6 pb-2">
                    <h3 className="text-sm font-medium text-gray-400 font-mono">// people interested in your projects</h3>
                    ...
                    {Object.keys(projectsWithApplicants).length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 border-2 border-dashed border-amber-500/20 rounded-lg">
                            <div className="bg-amber-500/10 p-4 rounded-md mb-3">
                                <Building className="w-10 h-10 text-amber-500/70" />
                            </div>
                            <p className="text-gray-400 text-lg font-mono">applicants.length === 0</p>
                            <p className="text-gray-500 text-sm mt-1 font-mono">// when developers apply to your projects, they will appear here</p>
                        </div>
                    ) : (
                            ...
                            {Object.entries(projectsWithApplicants).map(([projectName, applicants]) => (
                                <div key={projectName} className="mb-8">
                                    <div className="flex items-center gap-2 mb-4">
                                        <div className="h-8 w-1 bg-amber-500 rounded-sm"></div>
                                        <h3 className="text-xl font-semibold text-white font-mono">
                                            {projectName}
                                        </h3>
                                        <span className="ml-auto text-xs text-gray-400 bg-amber-500/10 px-2 py-1 rounded font-mono">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6">
                                        {applicants.map((applicant) => (
                                            <div
                                                ...
                                                className="bg-gray-950 p-6 rounded-lg border border-amber-500/30 hover:border-amber-500/50 transition-all"
                                            >
                                                <div className="flex justify-between items-center mb-3">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <h4 className="text-lg font-semibold text-white font-mono">{applicant.applicantName}</h4>
                                                            <p className="text-gray-400 text-sm font-mono">@{applicant.applicantUsername}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div>
                                                        {applicant.applicationStatus === "ACCEPTED" && (
                                                            <span className="flex items-center text-green-400 text-sm bg-green-900/20 px-2 py-1 rounded-md border border-green-600/30 font-mono">
                                                                <CheckCircle className="w-4 h-4 mr-1" /> ACCEPTED
                                                            </span>
                                                        )}
                                                        {applicant.applicationStatus === "PENDING" && (
                                                            <span className="flex items-center text-amber-400 text-sm bg-amber-900/20 px-2 py-1 rounded-md border border-amber-600/30 font-mono">
                                                                <Clock className="w-4 h-4 mr-1" /> PENDING
                                                            </span>
                                                        )}
                                                        {applicant.applicationStatus === "REJECTED" && (
                                                            <span className="flex items-center text-red-400 text-sm bg-red-900/20 px-2 py-1 rounded-md border border-red-600/30 font-mono">
                                                ...
                                                {/* Action Buttons */}
                                                <div className="flex justify-between items-center mt-4 pt-3 border-t border-amber-500/20">
                                                    <Link
                                                        to={`/user/profile/view/${applicant.applicantUsername}`}
                                                        className="text-amber-400 hover:text-amber-300 flex items-center gap-1 transition font-medium font-mono"
                                                    >
                                                        profile.view() <ArrowRight className="w-4 h-4" />
                                                    </Link>
                                                    ...
                                                    {applicant.applicationStatus === "PENDING" && (
                                                        <div className="flex gap-2">
                                                            <button
                                                                ...
                                                                className="bg-amber-500 hover:bg-amber-400 text-black px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-1 font-mono"
                                                            >
                                                                <CheckCircle className="w-4 h-4" /> applicant.accept()
                                                            </button>
                                                            <button
                                                                onClick={() => handleApplicationAction(applicant.projectId, applicant.applicationId, false)}
                                                                className="bg-gray-950 border border-amber-500/50 hover:border-amber-500 text-amber-400 px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-1 font-mono"
                                                            >
                                                                <XCircle className="w-4 h-4" /> applicant.reject()
                                                            </button>
                                                        </div>
                                                    )}
                                                </div>
