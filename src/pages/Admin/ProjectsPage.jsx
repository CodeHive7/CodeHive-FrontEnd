 ...
 * Example: chunkProjects([1,2,3,4,5], 3) => [ [1,2,3], [4,5] ]
 */
function chunkProjects(projects, size = 3) {
    const chunks = [];
...
export default function ProjectsPage() {
    ...
    // Chunk the projects into groups of up to 3
        ...
        <div className="min-h-screen bg-[#0A0B14] text-white py-8">
            <div className="container mx-auto px-4">
                ...
                {loading ? (
                    ...
                    // Map over each "block" of up to 3 projects
                    chunkedProjects.map((chunk, blockIndex) => {
                        // Determine how many columns we need based on how many items are in this chunk
                        const colCount =
                            chunk.length === 1 ? "grid-cols-1" : "grid-cols-2";

                        return (
                            <div key={blockIndex} className={`grid ${colCount} gap-8 mb-8 w-full`}>
                                {chunk.map((project) => (
                                    <div
                                        key={project.id}
